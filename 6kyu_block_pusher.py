    ...
    for move in range(n):
        ...
        for pusher_index in pusher_indexes:
            try:
                ...
                next_pusher = any(index > pusher_index and index < space_pos for index in pusher_indexes)
                if next_pusher:
                    next_pushers = [index for index, value in enumerate(lst[pusher_index+1:space_pos]) if value == pusher]
